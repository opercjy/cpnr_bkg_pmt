...
* **왜 도커(Docker)를 사용하는가?**
  Geant4와 ROOT는 운영체제 및 시스템 라이브러리(C++ 버전, Qt 등) 의존성이 매우 높습니다. 로컬 PC에 직접 설치할 경우 발생하는 수많은 충돌 에러를 원천 차단하기 위해, 전 세계 어디서든 100% 동일한 런타임 환경을 보장하는 도커 컨테이너를 사용합니다.

* **왜 광학 물리(Optical Physics)는 켜두고 궤적(Trajectory) 저장만 차단하는가?**
  NaI(Tl) 결정 내에서 발생한 광자가 테플론의 람베르트 난반사(Lambertian Diffuse Reflection)와 결정 내부의 레일리 산란(Rayleigh Scattering)을 거쳐 PMT 광전 음극(QE 28%)에 도달하는 미시적 상호작용은, 검출기의 집광 효율과 공간적 비대칭성을 결정짓는 핵심입니다. 이를 상수 곱하기나 단순 역산으로 퉁치면 초저방사능 실험의 물리적 무결성이 심각하게 훼손됩니다. 따라서 **광학 물리 엔진은 100% 풀가동**하여 빛의 흐름을 완벽히 모사하되, 수백억 개의 광자 궤적 데이터를 RAM에 기록하다가 시스템이 붕괴(OOM)되는 것을 막기 위해 커스텀 UI 커맨드(`/RI_Sim/tracking/storeOpticalTraj false`)를 도입하여 **메모리 할당만 선택적으로 차단**하는 아키텍처를 채택했습니다.

* **왜 '절대 시간'이 아닌 '상대적 트리거 타임 윈도우(Relative Trigger Time Window)'를 적용했는가?**
  U-238 등 NORM(자연방사능) 동위원소는 붕괴 사슬을 거치며 수 일에서 수억 년에 걸쳐 에너지를 방출합니다. Geant4의 기본 `RadioactiveDecay`는 이 모든 시간을 추적하므로, 커널에서 단순한 절대 시간 컷오프(예: 10 µs 이후 버림)를 적용하면 정상적인 붕괴 신호마저 모조리 소멸하는 '시간의 역설'이 발생합니다. 이를 타파하기 위해, 에너지가 검출기에 처음 침적되는 순간을 **DAQ 시스템의 트리거($T_0$)**로 잡고, 그 시점으로부터 **10 µs 동안만 게이트(Gate)를 열어 에너지를 적분**하도록 `EventAction`을 재설계했습니다. 이는 실제 환경 방사능 계측 실험의 오프라인 데이터 획득 알고리즘을 전산상에서 100% 동일하게 모사한 결과입니다.
