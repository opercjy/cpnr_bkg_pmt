...
G4VPhysicalVolume* DetectorConstruction::Construct() {
    ...
    // 알파선 및 고밀도 에너지 침적에 대한 퀜칭(비선형 발광 감소) 효과 적용
    ...
    // =========================================================================
    // 2.5 eV ~ 3.5 eV (약 350 nm ~ 500 nm) 가시광선 스펙트럼 대역 모사
    std::vector<G4double> photonEnergy = {2.5 * eV, 3.0 * eV, 3.5 * eV};

    // 굴절률(1.85), 자체 흡수 길이(100cm), 발광 피크(415nm에서 1.0)
    ...
    // ★ [전문가 패치] 레일리 산란(Rayleigh Scattering)
    // 415nm 대역에서 NaI의 전형적인 산란 길이인 40cm를 적용하여 빛이 결정 내부에서 
    // 현실적으로 산란하며 PMT 집광 시간 및 효율에 물리적 영향을 주도록 합니다.
    std::vector<G4double> rayleighNaI = {40.*cm, 40.*cm, 40.*cm};

    G4MaterialPropertiesTable* mptNaI = new G4MaterialPropertiesTable();
    mptNaI->AddProperty("RINDEX", photonEnergy, rindexNaI);
    mptNaI->AddProperty("ABSLENGTH", photonEnergy, absNaI);
    mptNaI->AddProperty("RAYLEIGH", photonEnergy, rayleighNaI);
    mptNaI->AddProperty("SCINTILLATIONCOMPONENT1", photonEnergy, scintYield);
    
    // 광수율 (Light Yield): 1 MeV당 38,000개
    mptNaI->AddConstProperty("SCINTILLATIONYIELD", 38000. / MeV);
    
    // [Geant4 v11 필수 파라미터] 첫 번째 컴포넌트의 발광 비율 (1.0 = 100%)
    mptNaI->AddConstProperty("SCINTILLATIONYIELD1", 1.0); 
    
    mptNaI->AddConstProperty("RESOLUTIONSCALE", 1.0); // 포아송 요동 적용
    mptNaI->AddConstProperty("SCINTILLATIONTIMECONSTANT1", 250. * ns); // 형광 감쇠 시간 250ns
    ...
    // --- 경계면(Boundary) 표면 특성 ---
    // =========================================================================
    // ★ [전문가 패치] 테플론 반사체: UNIFIED 모델 적용
    // 실제 테플론 랩핑의 거친 미세표면을 모사하기 위해 UNIFIED 모델의 groundfrontpainted를 사용하여 
    // 빛이 닿는 즉시 98%의 확률로 완벽한 '람베르트 난반사(Diffuse Reflection)'를 일으키게 합니다.
    G4OpticalSurface* refSurface = new G4OpticalSurface("ReflectorSurface", unified, groundfrontpainted, dielectric_dielectric);
    ...
    // 광전 음극 (Photocathode): GLISUR 모델, QE 28%
    G4OpticalSurface* pmtSurface = new G4OpticalSurface("PhotocathodeSurface", glisur, polished, dielectric_metal);
    std::vector<G4double> refPMT = {0.0, 0.0, 0.0};  // 빛이 튕겨나가지 않고 침투함
    std::vector<G4double> effPMT = {0.28, 0.28, 0.28}; // 28% 양자 효율(QE) 적용
    ...
    // =========================================================================
    // 2. 기하 구조 생성 (Geometry Construction)
    // =========================================================================
    ...
    // World 볼륨은 렌더링 시 보이지 않도록 숨김 처리
    fLogicWorld->SetVisAttributes(G4VisAttributes::GetInvisible());
    return physWorld;
}

void DetectorConstruction::ConstructSDandField() {
    // PMT 광전 음극을 Sensitive Detector로 등록
